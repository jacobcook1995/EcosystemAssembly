...
function intstats()
    ...
    for i = 1:rps
        ...
        while Rn_end == false
            ...
            if Tind > 1
                ...
                cmb_no_selff[i,cnt] = no_selff[Tind]*(T1x)/Tg + no_selff[Tind-1]*(Tx2)/Tg
                cmb_no_selfc[i,cnt] = no_selfc[Tind]*(T1x)/Tg + no_selfc[Tind-1]*(Tx2)/Tg
                cmb_via_no_comp[i,cnt] = via_no_comp[Tind]*(T1x)/Tg + via_no_comp[Tind-1]*(Tx2)/Tg
                cmb_via_no_facl[i,cnt] = via_no_facl[Tind]*(T1x)/Tg + via_no_facl[Tind-1]*(Tx2)/Tg
                cmb_via_no_selff[i,cnt] = via_no_selff[Tind]*(T1x)/Tg + via_no_selff[Tind-1]*(Tx2)/Tg
                cmb_via_no_selfc[i,cnt] = via_no_selfc[Tind]*(T1x)/Tg + via_no_selfc[Tind-1]*(Tx2)/Tg
                cmb_st_comp[i,cnt] = st_comp[Tind]*(T1x)/Tg + st_comp[Tind-1]*(Tx2)/Tg
                cmb_st_facl[i,cnt] = st_facl[Tind]*(T1x)/Tg + st_facl[Tind-1]*(Tx2)/Tg
                cmb_st_selfc[i,cnt] = st_selfc[Tind]*(T1x)/Tg + st_selfc[Tind-1]*(Tx2)/Tg
                cmb_st_selff[i,cnt] = st_selff[Tind]*(T1x)/Tg + st_selff[Tind-1]*(Tx2)/Tg
            else
    ...
    for i = 1:length(times)
        ...
        sd_no_selff[i] = sqrt(sum((cmb_no_selff[inds,i] .- mn_no_selff[i]).^2)/(no_sims[i] - 1))
        sd_no_selfc[i] = sqrt(sum((cmb_no_selfc[inds,i] .- mn_no_selfc[i]).^2)/(no_sims[i] - 1))
        ...
        sd_st_selff[i] = sqrt(sum((cmb_st_selff[inds,i] .- mn_st_selff[i]).^2)/(no_sims[i] - 1))
        sd_st_selfc[i] = sqrt(sum((cmb_st_selfc[inds,i] .- mn_st_selfc[i]).^2)/(no_sims[i] - 1))
        # These should be calculated just for viable strains
        if no_via[i] > 1
            sd_via_no_comp[i] = sqrt(sum((cmb_via_no_comp[vinds,i] .- mn_via_no_comp[i]).^2)/(no_via[i] - 1))
            sd_via_no_facl[i] = sqrt(sum((cmb_via_no_facl[vinds,i] .- mn_via_no_facl[i]).^2)/(no_via[i] - 1))
            sd_via_no_selff[i] = sqrt(sum((cmb_via_no_selff[vinds,i] .- mn_via_no_selff[i]).^2)/(no_via[i] - 1))
            sd_via_no_selfc[i] = sqrt(sum((cmb_via_no_selfc[vinds,i] .- mn_via_no_selfc[i]).^2)/(no_via[i] - 1))
        else
    ...
    jldopen("Output/$(Np)Pools$(M)Metabolites$(Nt)Speciesd=$(d)/IntStats$(ims)Ims.jld","w") do file
        # Save times
