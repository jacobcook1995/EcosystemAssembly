...
# function to calculate the strength of facilation through a particular metabolite
function facl(Mn::Int64,ps::TOParameters,ms::Array{Microbe,1},pops::Array{Float64,1},concs::Array{Float64,1},
            ϕRs::Array{Float64,1},fcls::Array{Int64,2})
    # Find all reactions where metabolite is a product
    Pind = findall(x->x==Mn,ps.reacs.↦:Prd)
    # Preallocate counters for self-faciliation and facilitation
    selff = 0
    fcl = 0
    # Now loop over all pairs of strains (strain i facilitates strain j)
    for i = 1:length(ms)
        for j = 1:length(ms)
            # Check that faciliation interaction is present
            if fcls[i,j] > 0
                if i == j
                    ...
                    for k = 1:ms[i].R
                        ...
                        if ms[i].Reacs[k] ∈ Pind
                            ...
                            selff += pops[i]*qs(concs[r.Rct],concs[r.Prd],E,k,ms[i],ps.T,r)/NA
                        end
                    end
                else
                    # Loop over all of strain i's reaction
                    for k = 1:ms[i].R
                        ...
                        if ms[i].Reacs[k] ∈ Pind
                            ...
                            E = Eα(ϕRs[i],ms[i],k)
                            # Finally find flux for this reaction
                            fcl += pops[i]*qs(concs[r.Rct],concs[r.Prd],E,k,ms[i],ps.T,r)/NA
                        end
                    end
                end
            end
...
function comp(Mn::Int64,ps::TOParameters,ms::Array{Microbe,1},pops::Array{Float64,1},concs::Array{Float64,1},
            ϕRs::Array{Float64,1},cmps::Array{Int64,2})
        ...
        for j = 1:length(ms)
            ...
            if i == j
                ...
                for k = 1:ms[i].R
                    ...
                    E = Eα(ϕRs[i],ms[i],k)
                    # Finally find flux for this reaction
                    selfc += pops[i]*qs(concs[r.Rct],concs[r.Prd],E,k,ms[i],ps.T,r)/NA
                end
            ...
            elseif cmps[i,j] > 0
                ...
                for k = 1:ms[i].R
                    ...
                    if ms[i].Reacs[k] ∈ Sind
                        ...
                        E = Eα(ϕRs[i],ms[i],k)
                        # Finally find flux for this reaction
                        cmp += pops[i]*qs(concs[r.Rct],concs[r.Prd],E,k,ms[i],ps.T,r)/NA
                    end
                end
            end
        end
    ...
    for i = 1:rps
        ...
        for j = 1:length(T)
            ...
            for k = 1:ps.M
                ...
                selff, fcl = facl(k,ps,ms[inds],pops[inds],concs,ϕRs[inds],fcls[inds,inds])
                st_selff[j] += selff
                st_facl[j] += fcl
                # Find strength of competition interactions (of both types) involving this metabolite
                selfc, cmp = comp(k,ps,ms[inds],pops[inds],concs,ϕRs[inds],cmps[inds,inds])
                st_selfc[j] += selfc
                st_comp[j] += cmp
            end
        end
        # Now just save the relevant data
        jldopen("Output/$(Np)Pools$(M)Metabolites$(Nt)Speciesd=$(d)/IntsRun$(i)Data$(ims)Ims.jld","w") do file
