...
function twostep()
    ...
    for i = 1:N
        ...
        G1[i] = Rgas*T*(log(Q1)-log(Keq1[i]))
        G2[i] = Rgas*T*(log(Q2)-log(Keq2))
        # Save extra stuff for plotting
        rK2s[i] = rK2
        rK4s[i] = rK4
        Bees[i] = cB
    end
    ...
    theme(:wong2,dpi=200)
    plot(G1/GT,fs,label="",ylabel="Overall reaction flux",xlabel="Fraction dissipated on first step")
    savefig("Output/test1.png")
    plot(G1/GT,rK2s,label="")
    savefig("Output/test2.png")
    plot(G1/GT,rK4s,label="")
    savefig("Output/test3.png")
    plot(G1/GT,Bees,label="")
...
function twostep2()
    ...
    cC = 1.0
    # Sub non-varying parameters into B
    Bs = subs.(Bs,E10=>E0,E20=>E0,A=>cA,C=>cC,k1=>rk1,k2=>rk2,k3=>rk3,k4=>rk4)
    # Overall equlbrium constant, and reaction quoitent
    Keq = 1.0
    Q = cC/cA
    # Make list of equilbrium constants to search
    ...
    for i = 1:N
        ...
        for j = 1:M
            ...
            exr4 = E2*B*k3 - K3*E2X
            # Sub in all values to find value of the flux
            fs[i,j] = subs(exr4,B=>cB,C=>cC,E20=>E0,k3=>rk3,K3=>rK3,k4=>rk4,K4=>rK4)
            # Now calculating reaction quotients for each step
            Q1 = cB/cA
            Q2 = cC/cB
            # Use to calculate free energy dissipated at each step
