...
export initialise_prot, initialise_prot_M, initialise_prot_T, initialise_prot_fix, initialise_prot_KΩ, initialise_prot_gl
export initialise_ρ
...
function initialise_prot_KΩ(ps::ProtParameters,KΩ::Float64)
    # Now make the parameter set
    ps = make_ProtParameters(ps.MC,ps.γm,ps.T,ps.η,ps.KS,ps.kr,ps.kc,ps.ρ,ps.Kγ,ps.d,ps.Pb,ps.ϕH,KΩ,ps.fd,ps.r,ps.n,ps.δ,ps.κ)
    return(ps)
end
...
function initialise_prot_gl(γm::Float64,ΔG::Float64)
    ...
    # The reversibility factor remains the same as previously
    kr = 10.0
    # Considering batch culture so want this to be baiscally zero
