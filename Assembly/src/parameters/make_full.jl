...
function fix_reactions(O::Int64, M::Int64, μrange::Float64, T::Float64)
    @assert O==2 * M - 3 "Miscalculated the number of reactions expected"
    # preallocate output
    RP = zeros(Int64, O, 2)
    ΔG = zeros(O)
...
# function to generate set of three strains that interact thermodynamically
function initialise_syn_trio(kc::Float64, KS::Float64, kr::Float64)
    # Trio of strains
    N = 3
    # Three metabolites
    M = 3
    # Correspondingly 3 reactions
    O = 3
    # One reaction per strain
    R = 1
    # Assume that temperature T is constant at 20°C
    T = 293.15
    # Cell mass is taken from Bremer H, Dennis P (1996) Modulation of chemical
    ...
    # Then construct microbes
    for i in 1:N
        # Choose specific reactions and eta values for specific microbes
        if i == 1
            Reacs = [1]
            η = [3.0]
        elseif i == 2
        ...
        elseif i == 3
            ...
            error("too many strains provided, need a trio")
        end
        # Make vectors of the (fixed) kinetic parameters
        kcs = [kc]
        KSs = [KS]
...
# function to generate set of three strains that interact thermodynamically
function initialise_pol_duo(kc::Float64, KS::Float64, kr::Float64)
    # Duo of strains
    N = 2
    # Three metabolites
    M = 3
    ...
    for i in 1:N
        ...
        elseif i == 2
            ...
            error("too many strains provided, need a duo")
        end
        # Make vectors of the (fixed) kinetic parameters
        kcs = [kc]
        KSs = [KS]
