...
function full_dynamics!(
    ...
    for i = 1:length(ms)
        ...
        if x[i] <= 1e-5
            ...
            dx[length(ms)+ps.M+i] =
                J - (ms[i].MC * χs(ϕR, ms[i]) + x[length(ms)+ps.M+i]) * λ
        end
    end
    ...
    for i = 1:length(ms)
        ...
        for j = 1:ms[i].R
            ...
            dx[length(ms)+ps.reacs[ms[i].Reacs[j]].Prd] +=
                rate[i, ms[i].Reacs[j]] * x[i] / NA
            # and decrease the reactant
            dx[length(ms)+ps.reacs[ms[i].Reacs[j]].Rct] -=
                rate[i, ms[i].Reacs[j]] * x[i] / NA
        end
    end
