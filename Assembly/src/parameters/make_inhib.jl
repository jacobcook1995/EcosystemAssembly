...
export initialise_pab
function initialise_pab()
    N = 2
    M = 3
    O = 3
    # Assume that temperature T is constant at 20°C
    ...
    # Preallocate vector of reactions
    reacs = Array{Reaction,1}(undef,O)
    reacs[1] = make_Reaction(1,1,2,-1.5e5)
    reacs[2] = make_Reaction(2,1,3,-3e5)
    reacs[3] = make_Reaction(3,2,3,-1.5e5)
    # Preallocate vector of microbes
    mics = Array{Microbe,1}(undef,N)
    # Each microbe has one reaction
    η = 0.5
    ...
    R1 = 2
    R2 = 3
    mics[1] = make_Microbe(m[1],g[1],R1,[1,3],η*ones(R1),qm*ones(R1),KS*ones(R1),kr*ones(R1))
    mics[2] = make_Microbe(m[2],g[2],R2,[1,2,3],η*ones(R2),qm*ones(R2),KS*ones(R2),kr*ones(R2))
    # Now make the parameter set
    ps = make_InhibParameters(N,M,O,T,κ,δ,reacs,mics)
    return(ps)
end
