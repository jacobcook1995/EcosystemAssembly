...
function int_vis()
    ...
    # Initial conditions for no syntrophy case
    pop = [1e3, 1e3, 0.0]
    ...
    # Initial conditions for with syntrophy case
    pop = [1e3, 1e3, 1e3]
    # Then run this simulation
    ...
    p1 = plot(xlabel = "Time (s)", ylabel = "Population", yaxis = :log10,
              legend = :bottomright)
    plot!(p1, Tns, Cns[:, 1:2], labels = ["Species A" "Species B"],
          title = "No syntrophy interaction")
    # Add annotation
    ...
    p3 = plot(xlabel = "Time (s)", ylabel = "Population", yaxis = :log10,
              legend = :bottomright)
    plot!(p3, Tws, Cws[:, 1:3], labels = ["Species A" "Species B" "Species C"],
          title = "Syntrophy interaction")
    # Add annotation
    ...
    p3 = plot(xlabel = "Time (s)", ylabel = "Population", yaxis = :log10,
              legend = :bottomright)
    plot!(p3, Twp, Cwp[:, 1:2], labels = ["Species A" "Species B"],
          title = "Pollution interaction")
    # Add annotation
