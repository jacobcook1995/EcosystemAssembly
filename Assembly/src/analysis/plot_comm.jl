...
function net_vis()
    ...
    ϕpF = []
    efF = []
    # minimum product to substrate ratio (to calculate) the efficiency
    mr = 1e-2
    ...
    for i in 1:nR
        ...
        bstf = zeros(ps.N)
        # reaction that achieves this
        bstr = zeros(Int64, ps.N)
        # Highest flux through one strain
        hghf = zeros(ps.O)
        # microbes that achieves that flux
    ...
    fRT /= nR
    # Then plot
    bar(fRT, xticks = (1:(ps.O), xs), label = "", xlabel = "Reaction",
        ylabel = "Average flux", title = tl)
    savefig("Output/$(Rl)-$(Ru)$(syn)/AverageReacsType$(Rl)-$(Ru)$(syn).png")
    # Then find and plot average mass renormalised flux
    fRTm /= nR
    bar(fRTm, xticks = (1:(ps.O), xs), label = "", xlabel = "Reaction",
        ylabel = "Mass renormalised average flux", title = tl)
    ...
    bar(perc, label = "", ylabel = "Average percentage of flux through top strain",
        title = tl, xlabel = "Reaction", xticks = (1:(ps.O), xs))
    savefig("Output/$(Rl)-$(Ru)$(syn)/PercentFluxType$(Rl)-$(Ru)$(syn).png")
    bar(perc2, label = "", ylabel = "Percentange of flux (ignoring singles)", title = tl,
        xlabel = "Reaction", xticks = (1:(ps.O), xs))
    ...
    # Then plot as bar charts, with initial distribution included
...
function plt_trdff()
    ...
    ps = load(pfile, "ps")
    out = load(ofile, "out")
    # Pick first reaction of the first microbe (guaranteed to exist)
...
function basic_info()
    ...
    hmb = zeros(Int64, rps) # Lowest (in energy hierarchy) metabolite
    ...
    for i in 1:rps
        ...
        # Loop over microbes, to find efficiencies
        for j in 1:(ps.N)
            # Find vector of ΔG0 values
            dG = ps.reacs[ps.mics[j].Reacs] .↦ :ΔG0
            # Use to calculate percentage dissipated (under standard conditions)
    ...
    for i in 1:(M - 1)
        ...
        elseif fnd[i] == true
            scatter!(p, [i / (M - 1)], [mean(avR[i])], label = "")
        end
    end
    savefig(p, "Output/$(Rl)-$(Ru)$(syn)$(Ni)$(en)/GenvsDiv$(Rl)-$(Ru)$(syn)$(Ni).png")
    return (nothing)
end
...
function multi_sets()
    ...
    for i in 1:Ns
        ...
        while vld == false
            ...
            if rpt == true
                println("Parameter set repeated, please enter an original one")
            elseif ~isdir("Data/$(Rl)-$(Ru)$(syn)$(Ni)$(en)")
                println("Parameter set doesn't exist please re-enter")
            else
                vld = true
                Rls[i] = Rl
                Rus[i] = Ru
...
function plot_survivors()
    # Check that sufficient arguments have been provided
    if length(ARGS) < 7
        error("Insufficient inputs provided (looking for 7)")
    end
    # Preallocate the variables I want to extract from the input
    Rl = 0
    Ru = 0
        ...
        for j in eachindex(ens)
            ...
            for k in 1:ips
                ...
                if T[ind] == Ts[k]
                    ...
                    svs[k, i, j] = (1 - dT) * count(x -> x > 0.0, C[ind, 1:Ni]) +
                                   dT * count(x -> x > 0.0, C[ind - 1, 1:Ni])
                    dv[k, i, j] = (1 - dT) *
                                  count(x -> x > 0.0, C[ind, (Ni + 1):(Ni + ps.M - 1)]) +
                                  dT *
                                  count(x -> x > 0.0, C[ind - 1, (Ni + 1):(Ni + ps.M - 1)])
    ...
    savefig(p, "Output/$(Rl)-$(Ru)$(syn)$(Ni)/TotalAbTime$(Rl)-$(Ru)$(syn)$(Ni).png")
    return (nothing)
end
