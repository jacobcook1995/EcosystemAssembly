...
function convert_Parameters(ps::MarsParameters)
    ...
    c = ps.c
    # Neither should the vectors m, δ and κ
    m = ps.m
    δ = ps.δ
    κ = ps.κ
    return(make_InhibParameters(N,M,c,m,δ,κ))
end

"""
    make_InhibParameters(N::Int64,M::Int64,c::Array{Float64,2},m::Vector{Float64},κ::Vector{Float64},δ::Vector{Float64})
...
"""
function make_InhibParameters(N::Int64,M::Int64,c::Array{Float64,2},m::Vector{Float64},κ::Vector{Float64},δ::Vector{Float64})
...
return(InhibParameters(N,M,c,m,κ,δ))
end
