...
# function to find krates based on max rates, and combined value of K1K2
function kratesALT(maxrate::Float64,k1k2::Float64,k2::Float64,KeQ::Float64,E0::Float64,m::Float64,η::Float64)
    # Find k1
    k1 = k1k2/k2
    # Find value of K1K2
    K1K2 = k1k2/KeQ
...
function fixk1k2()
    ...
    maxrate = 1.0e6
    K1K2 = 1.0e7
    # Fixed k1k2 based on standard Temp
    ...
    for j = 1:length(Temps)
        ...
        for i = 1:length(k2)
            # Find k rates from function
            k1, K1, K2 = kratesALT(maxrate,k1k2,k2[i],KeQ[j],E0,m,η)
            qm[i,j], KS[i,j], _, kr[i,j] = qKmY(k1,K1,k2[i],K2,E0)
            QT[i,j] = Qineq(η,qm[i,j],m,kr[i,j],KeQ[j])
            _, _, Ns[i,j] = stead(KS[i,j],kr[i,j],η,qm[i,j],m,CO,α,δ,θ)
        end
    ...
    Lqm = L"q_m"
    plot(qm*1e17,KS,xlabel="$(Lqm) (10^-17)")
    savefig("Output/ALTqmvsKS.png")
    plot(qm*1e17,kr,xlabel="$(Lqm) (10^-17)")
    savefig("Output/ALTqmvskr.png")
    plot(qm*1e17,Ns,xlabel="$(Lqm) (10^-17)")
    savefig("Output/ALTqmvsNs.png")
    plot(qm*1e17,QT,xlabel="$(Lqm) (10^-17)")
    savefig("Output/ALTqmvsQT.png")
    return(nothing)
end
