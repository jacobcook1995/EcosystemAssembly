...
function twostep()
    ...
    Bs = subs.(Bs, E10 => cE1, E20 => cE2, A => cA, C => cC, k1 => rk1, k2 => rk2,
               k3 => rk3, k4 => rk4)
    # Overall equilibrium constant, and reaction quotient
    Keq = 1.0
    Q = cC / cA
    # Make list of equilibrium constants to search
    Keq1 = [collect(0.0001:0.0001:0.001); collect(0.001:0.001:0.01); collect(0.01:0.01:0.1);
            collect(0.1:0.1:1.0); collect(1.0:1.0:10.0)]
    ...
    fs = zeros(N, M)
    G1 = zeros(N, M)
    # Loop over a range of equilibrium constant values
    for i in 1:N
        Keq2 = Keq / Keq1[i]
        for j in 1:M
            ...
            fs[i, j] = subs(exr4, B => cB, C => cC, E20 => cE2, k3 => rk3, K3 => rK3,
                            k4 => rk4, K4 => rK4)
    ...
    theme(:wong2, dpi = 200)
    plot(G1 / GT, fs, labels = lbs, ylabel = "Overall reaction flux",
         xlabel = "Fraction dissipated on first step")
