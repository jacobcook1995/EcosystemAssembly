...
# WORK OUT IF THIS, AND OTHER STUFF IS ACTUALLY NEEDED
...
function figure7(rps::Int64)
    ...
    mn_fr_ΔG1 = load(sfile,"mn_fr_ΔG1")
    mn_fr_ΔG2 = load(sfile,"mn_fr_ΔG2")
    ...
    sd_fr_ΔG1 = load(sfile,"sd_fr_ΔG1")
    sd_fr_ΔG2 = load(sfile,"sd_fr_ΔG2")
    ...
    plot!(p3,title="Variation with free energy",ylabel="Maximum ribsome fraction factor ($(L"\omega"))")
    ...
    # Make plot objects
    p1 = plot(xlabel="Times (s)",xlim=(-Inf,2.5e6),legend=:right,ylabel=L"\eta",title="ATP yield")
    p2 = plot(xlabel="Times (s)",xlim=(-Inf,2.5e6),ylim=(0.55,Inf),legend=false)
    plot!(p2,ylabel="Fraction of free-energy transduced",title="Average reaction efficiency")
    p3 = plot(xlabel="Times (s)",xlim=(-Inf,2.5e6),ylim=(0.5,Inf),legend=:bottomright)
    plot!(p3,ylabel="Fraction of free-energy transduced",title="Reaction efficency by reaction type")
    p4 = plot(xlabel="Times (s)",xlim=(-Inf,2.5e6),ylim=(0.4,Inf),legend=false)
    ...
    plot!(p1,times,mn_via_η,ribbon=se_via_η,color=a[1],label="")
    plot!(p2,times,mn_fr_ΔG,ribbon=se_fr_ΔG,color=a[1])
    plot!(p3,times,mn_fr_ΔG1,ribbon=se_fr_ΔG1,color=b[1],label="1-step")
    plot!(p3,times,mn_fr_ΔG2,ribbon=se_fr_ΔG2,color=b[3],label="2-step")
    plot!(p4,times,mn_av_steps.-1,ribbon=se_av_steps,color=a[1])
    # Save figures to this directory
    savefig(p1,"Output/Fig7/Eta.png")
    savefig(p2,"Output/Fig7/Efficency.png")
    savefig(p3,"Output/Fig7/CompEfficencies.png")
    savefig(p4,"Output/Fig7/Steps.png")
    # Plot all graphs as a single figure
    pt = plot(p1,p2,p3,p4,layout=(2,2),size=(1200,800),margin=5.0mm)
    savefig(pt,"Output/Fig7/temp.png")
    return(nothing)
end
