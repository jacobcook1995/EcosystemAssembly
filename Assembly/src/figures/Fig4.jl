...
# WORKS AT THE MOMENT BECAUSE OF FUNCTION OVERLOAD, BUT IN THE END ONLY ONE CAN REMAIN
function figure4(Rl::Int64,Ru::Int64,syn::Bool,ens::Array{String,1},rps::Int64,Ni::Int64,fT::Float64,sT::Float64)
    println("Compiled!")
    # Check I haven't provided stupid times
    if fT <= 0.0 || fT > 1.0
        error("final time can't be greater than 1, or less than zero")
    ...
    savefig("Output/Fig4/SvTime$(Rl)-$(Ru)$(syn)$(Ni).png")
    ...
    savefig("Output/Fig4/DvTime$(Rl)-$(Ru)$(syn)$(Ni).png")
    # Plot total abundances
    ...
    savefig("Output/Fig4/TotalAbTime$(Rl)-$(Ru)$(syn)$(Ni).png")
    return(nothing)
end

@time figure4(1,5,[true,false],["l","h"],250,250,2.5e-2,0.0)

# @time figure4(1,5,true,["l","i","h"],250,250,0.025,0.0)
