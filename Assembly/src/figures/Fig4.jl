...
function av_eff(pop::Array{Float64, 1}, conc::Array{Float64, 1}, ms::Array{MicrobeP, 1},
    ...
    # Set initial efficiency value
    efT = 0.0
    # Loop over indices
    for i in inds
        # Loop over number of reactions
        for j in 1:(ms[i].R)
            # Calculate proportional efficiency
            eff = -(ms[i].η[j] * ΔGATP) /
                  (ps.reacs[ms[i].Reacs[j]].ΔG0 + Rgas * ps.T * log(mr))
...
function figure4(Rl::Int64, Ru::Int64, syns::Array{Bool, 1}, ens::Array{String, 1},
    ...
    dv = zeros(ips, rps, L)
    tab = zeros(ips, rps, L)
    # New containers, for efficiencies and growth rates
    efs = zeros(ips, rps, L)
    λts = zeros(ips, rps, L)
    ...
    # Making my own palette
    pl = Array{RGBA, 1}(undef, L)
    # Define the 4 colours I want to show
    ...
    savefig(p3, "Output/Fig4/Efficiency.png")
    # Make label
    s1 = L"s^{-1}"
