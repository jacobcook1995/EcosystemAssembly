...
function figure6(rps::Int64,ims::Int64)
    ...
    plot!(p1,ylim=(1.0,6.0))
    p2 = plot(xlabel="Times (s)",xlim=(-Inf,5e7),ylim=(0.55,0.8),legend=false)
    plot!(p2,ylabel="Fraction of free-energy transduced",title="Average reaction efficiency")
    p3 = plot(xlabel="Times (s)",xlim=(-Inf,5e7),ylim=(2.0,2.75),legend=false)
    plot!(p3,ylabel="Average number of reaction steps",title="Relative frequency of reaction types")
    ...
    se_via_η_bw = sd_via_η_bw./sqrt.(no_via)
    # Then plot
    p4 = plot(times,mn_via_η_bw,ribbon=se_via_η_bw,color=a[3],label="high free-energy")
    plot!(p4,xlim=(-Inf,2.5e6),ylim=(1.0,6.0),title="ATP yield without immigration")
    ...
    px, py = annpos([0.0;5e7],[0.55;0.8],0.075,0.05)
    annotate!(p2,[px,py,text("B",17,:black)])
    px, py = annpos([0.0;5e7],[2.0;2.75],0.075,0.05)
