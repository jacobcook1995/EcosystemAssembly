...
# generating communities under different parametrisations
using Assembly
using JLD
...
function rb_assemble()
    ...
    for i in Rs:rps
        ...
        elseif opt == 3
            # Option 3 changes the size of the housekeeping fraction
            ϕH = 0.7
        ...
        jldopen("Paras/$(title_op)/$(Rl)-$(Ru)$(syn)$(en)/ParasReacs$(Rl)-$(Ru)Syn$(syn)Run$(i)Ns$(N).jld",
                "w") do file
            write(file, "ps", ps)
        end
        # Find starting time
        ...
        jldopen("Data/$(title_op)/$(Rl)-$(Ru)$(syn)$(en)/ExtinctReacs$(Rl)-$(Ru)Syn$(syn)Run$(i)Ns$(N).jld",
                "w") do file
            write(file, "ded", ded)
        end
        ...
        # and the full output
        jldopen("Data/$(title_op)/$(Rl)-$(Ru)$(syn)$(en)/OutputReacs$(Rl)-$(Ru)Syn$(syn)Run$(i)Ns$(N).jld",
                "w") do file
            # Save final output
            write(file, "out", out)
