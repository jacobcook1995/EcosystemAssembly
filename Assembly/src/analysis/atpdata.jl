...
function dadt!(dx::Array{Float64, 1}, x::Array{Float64, 1}, m::MicrobeP, P::Float64,
               t::Float64)
    # Need to find E
    E = Eα(x[2], m, 1)
    # Assume far from equilibrium
    θs = 0.0
    # Find reaction rate
    qr = qs(m, x[3], P, E, θs)
    # Find rate of energy acquisition
    J = m.η[1] * qr
...
function eval_a(m::MicrobeP, S::Float64, P::Float64, ϕR0::Float64, a0::Float64,
                Tmax::Float64, tsave::Array{Float64, 1})
...
function chi_square(m::MicrobeP, S::Float64, P::Float64, ϕR0::Float64, Tmax::Float64,
                    tsave::Array{Float64, 1},
...
# function to use steepest descent to find optimal parameters to fit the empirical curves
function prot_fit(times::Array{Float64, 1}, pt::Float64, mA::Array{Float64, 1},
                  sdA::Array{Float64, 1}, Kγ::Float64 = 5e8,
    ...
    P = S / 100.0
    # Use parameters from literature (justified in make_full.jl)
    MC = 10^8
    γm = 1260.0 / 60.0
    n = zeros(Int64, 3)
    n[1] = 7459
    ...
    krs = [10.0]
    ϕP = [1.0]
    # No explicit reactions so η is a parameter with less consequence
    η = [2.5]
    ...
    while stab == false
        ...
        m = make_MicrobeP(MC, γm, ρ, Kγ, Pb, d, ϕH, KΩ, fd, R, Reacs, η, [kc], KSs, krs, n,
                          ϕP)
        ...
        mγu = make_MicrobeP(MC, γm, ρ, Kγu, Pb, d, ϕH, KΩ, fd, R, Reacs, η, [kc], KSs, krs,
                            n, ϕP)
        mγd = make_MicrobeP(MC, γm, ρ, Kγd, Pb, d, ϕH, KΩ, fd, R, Reacs, η, [kc], KSs, krs,
                            n, ϕP)
        mΩu = make_MicrobeP(MC, γm, ρ, Kγ, Pb, d, ϕH, KΩu, fd, R, Reacs, η, [kc], KSs, krs,
                            n, ϕP)
        mΩd = make_MicrobeP(MC, γm, ρ, Kγ, Pb, d, ϕH, KΩd, fd, R, Reacs, η, [kc], KSs, krs,
                            n, ϕP)
        mku = make_MicrobeP(MC, γm, ρ, Kγ, Pb, d, ϕH, KΩu, fd, R, Reacs, η, [kcu], KSs, krs,
                            n, ϕP)
        mkd = make_MicrobeP(MC, γm, ρ, Kγ, Pb, d, ϕH, KΩd, fd, R, Reacs, η, [kcd], KSs, krs,
                            n, ϕP)
    ...
    C, T = eval_a(m, S, P, ϕR0, minimum(admA), Tmax, tsave)
    # Recalculate mean and errors to get "true χ2"
    admA = mA * 6.02e23 / 1e9
    adsdA = sdA * 6.02e23 / 1e9
    χ2 = chi_square(m, S, P, ϕR0, Tmax, tsave, indm, admA, adsdA)
...
function atp_read()
    ...
    for i in 1:ni
        ...
        ts = sort(unique([data[i, 3, 1, 1]; data[i, 3, 2, 1]; data[i, 3, 3, 1];
                          data[i, 3, 4, 1]]))
    ...
    df = DataFrame(taxa = genus, kc = kcs, Kγ = Kγs, KΩ = KΩs, ϕR0 = ϕRs, χ_2 = χ2s,
                   valid = vld)
...
function atp_plot()
    ...
    for i in 1:ni
        ...
        _, _, _, _, _, C, T = prot_fit(ts, pt[i], mA, sdA, df.Kγ[i], df.KΩ[i], df.kc[i],
                                       df.ϕR0[i])
        # Plot final result
        plot(T .+ (pt[i] * 60.0), C[:, 1], label = "χ2 = $(round(df.χ_2[i],sigdigits=3))",
             title = "$(df.taxa[i])")
